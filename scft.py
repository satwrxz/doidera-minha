        ...
        if self.on_ground and self.state in (self.PATROL, self.CHASE, self.ATTACK):
            ...
            if on_edge:
                if self.state == self.PATROL:
                    ...
                    # Chase/Attack: tenta pular o gap
...
class Boss(Enemy):
    ...
    def update(self, platforms, player_rect, projectiles):
        ...
        if cycle > max(100,160-self.phase*20):
            ...
            self.glow_intensity = min(255, self.glow_intensity + 8)
        else:
        ...
        if cycle==0 and self.shoot_cd==0:
            ...
            self.glow=255; self.glow_intensity=255; self.shoot_cd=20
    ...
    def draw(self, surf, cx, cy):
        ...
        # Telegraphing glow
        if self.glow>0 or self.glow_intensity>0:
            g_alpha = max(self.glow//3, self.glow_intensity//2)
            draw_circle_alpha(surf,(255,80,80),(rx+w//2,ry+h//2),80,g_alpha)
...
class HUD:
    ...
    def draw_start_screen(self,surf):
        ...
        title_bob = int(math.sin(t * 2.0) * 10)

        title=ft.render("SHADOWCROFT",True,C_UI_TITLE)
        draw_rect_alpha(surf,C_CHK_GLOW,
            (SCREEN_W//2-title.get_width()//2-12,SCREEN_H//2-68+title_bob,
             title.get_width()+24,72),gl//4)
        surf.blit(title,(SCREEN_W//2-title.get_width()//2,SCREEN_H//2-62+title_bob))
...
def build_level(level_id=1, difficulty=1.0):
    ...
    if level_id == 1:
        ...
        # Zona 4 (Expandida): Novas plataformas e inimigos
        platforms.append(Platform(3000, 550, 400, 40, 1))
        platforms.append(Platform(3450, 500, 300, 40, 2))
        enemies.append(Enemy(3100, 510, 100, hp(4)))
        enemies.append(ShootingEnemy(3500, 460, 100, hp(3)))
        enemies.append(FlyingEnemy(3700, 400, hp(3), float_height=350))

        checkpoints.append(Checkpoint(3000, 514))
        orbs.append(HealthOrb(3500, 450))
    ...
    # ── LEVEL 2: Plataformas — corrigido, sem espinhos duplos ─
        ...
        # Zona 4 (Expandida): Desafio final de plataformas
        m_plats.append(MovingPlatform(3400, 500, 150, 22, 300, -100))
        platforms.append(Platform(3800, 580, 450, 40, 1))
        enemies.append(ShootingEnemy(3850, 540, 100, hp(4)))
        enemies.append(Enemy(4000, 540, 100, hp(4)))
        enemies.append(FlyingEnemy(4100, 450, hp(3), float_height=380))

        orbs.append(HealthOrb(3900, 520))
        checkpoints.append(Checkpoint(3800, 536))
    ...
    # ── LEVEL 3: Puzzles — switches, portões, espinhos alternados ─
        ...
        # Zona 1: Intro + switch (REDIRECIONADO)
        platforms.append(Platform(  0, 620, 600, 40, 0))
        platforms.append(Platform(600, 560, 300, 30, 1))
        switches.append(Switch(600, 520))  # Switch em x=600
        gates.append(Gate(1100, 340, 40, 220)) # Portão em x=1100
        ...
        platforms.append(Platform(1200, 540, 150, 28, 1, hazard=True, hazard_timed=True, hazard_offset=0))
        platforms.append(Platform(1400, 540, 150, 28, 1, hazard=True, hazard_timed=True, hazard_offset=60))
        platforms.append(Platform(1600, 600, 300, 40, 0))

        enemies.append(ShootingEnemy(800, 520, 80, hp(3)))
        enemies.append(Enemy(900, 520, 60, hp(4)))
        enemies.append(ShootingEnemy(1250, 500, 60, hp(3)))
        enemies.append(FlyingEnemy(1400, 430, hp(2), float_height=380))
        enemies.append(Enemy(1650, 560, 80, hp(4)))

        orbs.append(HealthOrb(1450, 470))
        checkpoints.append(Checkpoint(1200, 516))

        # Zona 3: Moving platforms + combinação
        m_plats.append(MovingPlatform(1950, 540, 140, 22, 0, -160))
        platforms.append(Platform(2150, 580, 320, 40, 0))
        m_plats.append(MovingPlatform(2530, 560, 130, 22, 180, 0))
        platforms.append(Platform(2760, 560, 300, 30, 2))
        platforms.append(Platform(3120, 600, 540, 40, 0))

        enemies.append(Enemy(2000, 500, 80, hp(4)))
        enemies.append(ShootingEnemy(2180, 540, 80, hp(3)))
        enemies.append(FlyingEnemy(2400, 430, hp(3), float_height=370))
        enemies.append(Enemy(2580, 520, 70, hp(4)))
        enemies.append(Enemy(2800, 520, 80, hp(4)))
        enemies.append(ShootingEnemy(2900, 520, 70, hp(3)))
        enemies.append(Enemy(3160, 560, 120, hp(4)))
        enemies.append(FlyingEnemy(3300, 480, hp(2), float_height=400))
        enemies.append(Enemy(3400, 560, 100, hp(4)))

        # Zona 4 (Expandida): Puzzle final
        switches.append(Switch(3800, 560))
        gates.append(Gate(4200, 340, 40, 250))
        platforms.append(Platform(3700, 600, 600, 40, 1))
        platforms.append(Platform(4400, 580, 400, 40, 0))
        enemies.append(ShootingEnemy(4000, 560, 100, hp(4)))
        enemies.append(Enemy(4450, 540, 120, hp(5)))

        checkpoints.append(Checkpoint(3750, 556))
    ...
    # ── LEVEL 4: A Ascensão — desafio de pulo vertical ───────
        ...
        # Zona 4 (Expandida): Ascensão final
        m_plats.append(MovingPlatform(4000, 500, 150, 22, 300, -200))
        platforms.append(Platform(4400, 550, 800, 40, 2))
        enemies.append(ShootingEnemy(4500, 510, 120, hp(5)))
        enemies.append(Enemy(4700, 510, 100, hp(5)))
        enemies.append(FlyingEnemy(4900, 400, hp(4), float_height=350))

        checkpoints.append(Checkpoint(4400, 506))
    ...
    # ── LEVEL 5: O Boss Final — arena corrigida ───────────────
        ...
        # Extensão do chão da arena para comportar o boss e o caminho para o goal
        ...
        # O portão da arena agora é um Boss Gate em ARENA_X2
...
class Game:
    ...
    def update(self):
        ...
        # Level 5 Boss Gate logic
        if self.current_level == 5 and self.hud.boss_ref:
            b = self.hud.boss_ref
            b_dead_state = getattr(Enemy, 'DEAD', 'dead')
            if getattr(b, 'state', '') == b_dead_state and getattr(b, 'dead_timer', 0) >= 40:
                sw_active = True
