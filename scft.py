...
def build_level(level_id=1, difficulty=1.0):
    ...
    # ── LEVEL 5: O Boss Final — arena corrigida ───────────────
        ...
        gates.append(Gate(ARENA_X2, 0, 40, 900))              # portão direito arena (abre ao matar boss)
...
class Game:
    ...
    def update(self):
        ...
        # Portão especial da Arena no Level 5 (abre ao matar boss)
        if self.current_level==5 and self.hud.boss_ref:
            dead_timer = getattr(self.hud.boss_ref, "dead_timer", 0)
            if dead_timer > 40: sw_active = True
