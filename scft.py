...
class HUD:
    ...
    def draw_reset_ui(self, surf, reset_timer, reset_msg_timer):
        if reset_timer > 0:
            bw, bh = 200, 10
            bx, by = SCREEN_W//2 - bw//2, 600
            pygame.draw.rect(surf, (50, 20, 20), (bx, by, bw, bh))
            pygame.draw.rect(surf, (255, 50, 50), (bx, by, int(bw * (reset_timer/120)), bh))
            txt = self.font_sm.render("Segure K para resetar...", True, (255, 100, 100))
            surf.blit(txt, (SCREEN_W//2 - txt.get_width()//2, by - 20))

        if reset_msg_timer > 0:
            txt = self.font_md.render("Progresso Resetado!", True, (100, 255, 100))
            surf.blit(txt, (SCREEN_W//2 - txt.get_width()//2, 600))
...
class Game:
    ...
    def update(self):
        ...
        # Portão do boss abre se ele morrer (espera animação)
        if self.current_level==5 and self.hud.boss_ref:
            b = self.hud.boss_ref
            dead_s = getattr(b, 'DEAD', 'dead')
            if getattr(b, 'state', '') == dead_s and getattr(b, 'dead_timer', 0) >= 40:
                sw_active = True
