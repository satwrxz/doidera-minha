...
class Enemy:
    ...
    def update(self, platforms, player_rect):
        ...
        # Ledge jumping logic (CHASE/ATTACK) or reverse (PATROL)
...
def build_level(level_id=1, difficulty=1.0):
    ...
    if level_id == 1:
        ...
        # Expanded Area L1 (x > 2700)
        platforms.append(Platform(2960, 500, 200, 30, 2))
        platforms.append(Platform(3200, 420, 180, 24, 0))
        platforms.append(Platform(3450, 550, 400, 40, 1))
        enemies.append(Enemy(3000, 460, 60, hp(4)))
        enemies.append(FlyingEnemy(3250, 380, hp(3), float_height=300))
        enemies.append(Enemy(3550, 510, 80, hp(4)))
        enemies.append(ShootingEnemy(3700, 510, 70, hp(3)))
        orbs.append(HealthOrb(3250, 350))

        goal = LevelGoal(3800, 474)
    ...
    # ── LEVEL 2: Plataformas — corrigido, sem espinhos duplos ─
        ...
        # Expanded Area L2 (x > 2700)
        platforms.append(Platform(2800, 440, 200, 30, 1))
        m_plats.append(MovingPlatform(3050, 440, 140, 22, 200, -100))
        platforms.append(Platform(3350, 380, 250, 30, 0))
        platforms.append(Platform(3650, 520, 200, 30, 2, hazard=True, hazard_timed=True, hazard_offset=30))
        platforms.append(Platform(3900, 560, 500, 40, 1))

        enemies.append(Enemy(2850, 400, 60, hp(5)))
        enemies.append(FlyingEnemy(3100, 350, hp(3), float_height=280))
        enemies.append(ShootingEnemy(3400, 340, 80, hp(4)))
        enemies.append(Enemy(3950, 520, 100, hp(5)))
        enemies.append(ShootingEnemy(4100, 520, 80, hp(4)))
        enemies.append(FlyingEnemy(4250, 450, hp(3), float_height=380))

        orbs.append(HealthOrb(3400, 310))
        goal = LevelGoal(4300, 484)
    ...
    # ── LEVEL 3: Puzzles — switches, portões, espinhos alternados ─
        ...
        switches.append(Switch(600, 520))  # Updated Switch Position
        gates.append(Gate(1100, 340, 40, 220)) # Updated Gate Position
        ...
        # Expanded Area L3 (x > 2700)
        platforms.append(Platform(3000, 520, 250, 30, 1))
        platforms.append(Platform(3350, 450, 200, 30, 2, hazard=True, hazard_timed=True, hazard_offset=15))
        platforms.append(Platform(3650, 380, 200, 30, 0))
        m_plats.append(MovingPlatform(3950, 450, 140, 22, 200, 150))
        platforms.append(Platform(4250, 580, 500, 40, 1))

        enemies.append(Enemy(3050, 480, 80, hp(5)))
        enemies.append(FlyingEnemy(3400, 380, hp(4), float_height=300))
        enemies.append(ShootingEnemy(3700, 340, 80, hp(4)))
        enemies.append(Enemy(4300, 540, 100, hp(5)))
        enemies.append(FlyingEnemy(4500, 480, hp(4), float_height=400))
        enemies.append(ShootingEnemy(4650, 540, 80, hp(4)))

        orbs.append(HealthOrb(3700, 310))
        goal = LevelGoal(4600, 504)
    ...
    # ── LEVEL 4: A Ascensão — desafio de pulo vertical ───────
        ...
        # Expanded Area L4 (x > 2700)
        # Populate x > 4000 (old limit) and continue expansion
        platforms.append(Platform(4050, 480, 200, 30, 2))
        platforms.append(Platform(4350, 400, 200, 30, 0))
        m_plats.append(MovingPlatform(4650, 380, 150, 22, 0, 200))
        platforms.append(Platform(4900, 580, 400, 40, 1))

        enemies.append(Enemy(4100, 440, 60, hp(6)))
        enemies.append(ShootingEnemy(4400, 360, 80, hp(5)))
        enemies.append(FlyingEnemy(4600, 280, hp(4), float_height=200))
        enemies.append(Enemy(4950, 540, 120, hp(6)))
        enemies.append(ShootingEnemy(5100, 540, 80, hp(5)))

        orbs.append(HealthOrb(4400, 330))
    ...
    # ── LEVEL 5: O Boss Final — arena corrigida ───────────────
        ...
        # Expanded floor beyond ARENA_X2 by 400px
...
class Game:
    ...
    def _load_data(self):
        try:
            ...
            print(f"No save data found or error loading: {e}")
