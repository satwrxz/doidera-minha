        ...
        if self.on_ground and self.state in (self.PATROL, self.CHASE, self.ATTACK):
            ...
            if on_edge:
                if self.state == self.PATROL:
                    ...
                    # Chase or Attack: JUMP across gap (0.75 * JUMP_FORCE)
...
class MovingPlatform:
    ...
    def update(self,player):
        ...
        # Foot detection margin 4px for on_ground state
...
class HUD:
    ...
    def draw_start_screen(self,surf):
        ...
        # Floating animation using sine wave based on pygame.time.get_ticks()
        ...
        draw_rect_alpha(surf,C_CHK_GLOW,
            (SCREEN_W//2-title.get_width()//2-12,SCREEN_H//2-68 + float_y,
             title.get_width()+24,72),gl//4)
        surf.blit(title,(SCREEN_W//2-title.get_width()//2,SCREEN_H//2-62 + float_y))
...
def build_level(level_id=1, difficulty=1.0):
    ...
    if level_id == 1:
        ...
        # Zona Expandida (Memory Requirement)
        platforms.append(Platform(3000, 550, 400, 40, 1))
        platforms.append(Platform(3500, 500, 300, 40, 0))
        enemies.append(Enemy(3100, 510, 80, hp(4)))
        enemies.append(FlyingEnemy(3600, 420, hp(2), float_height=350))

        goal = LevelGoal(3880, 414)
    ...
    # ── LEVEL 2: Plataformas — corrigido, sem espinhos duplos ─
        ...
        # Zona Expandida
        platforms.append(Platform(3400, 560, 400, 40, 1))
        m_plats.append(MovingPlatform(3900, 500, 150, 22, 0, -200))
        enemies.append(ShootingEnemy(3500, 520, 80, hp(3)))
        enemies.append(Enemy(3650, 520, 100, hp(4)))
        enemies.append(FlyingEnemy(4000, 300, hp(2), float_height=250))

        goal = LevelGoal(4380, 474)
    ...
    # ── LEVEL 3: Puzzles — switches, portões, espinhos alternados ─
        ...
        # Zona Expandida
        platforms.append(Platform(3600, 550, 500, 40, 1))
        enemies.append(Enemy(3700, 510, 100, hp(5)))
        enemies.append(ShootingEnemy(3900, 510, 100, hp(4)))
        platforms.append(Platform(4200, 480, 400, 40, 0))
        enemies.append(FlyingEnemy(4400, 380, hp(3), float_height=300))

        goal = LevelGoal(4680, 404)
    ...
    # ── LEVEL 4: A Ascensão — desafio de pulo vertical ───────
        ...
        # Zona Expandida
        platforms.append(Platform(4200, 500, 400, 40, 1))
        m_plats.append(MovingPlatform(4700, 450, 150, 22, 0, -250))
        enemies.append(Enemy(4300, 460, 80, hp(5)))
        enemies.append(ShootingEnemy(4500, 460, 80, hp(4)))
        enemies.append(FlyingEnemy(4800, 250, hp(3), float_height=150))

        goal = LevelGoal(5080, 414)
    ...
    # ── LEVEL 5: O Boss Final — arena corrigida ───────────────
        ...
        # Arena floor extended to width 400 beyond ARENA_X2
        ARENA_X1, ARENA_X2 = 1400, 3600
        platforms.append(Platform(ARENA_X1, 560, ARENA_X2-ARENA_X1 + 400, 45, 0))

        # Re-adding logical wall platforms to prevent boss from falling (Safety Requirement)
        platforms.append(Platform(ARENA_X1-40, 0, 40, 900))   # Left wall
        platforms.append(Platform(ARENA_X2+400, 0, 40, 900)) # Right wall (at extended floor edge)
...
class Game:
    ...
    def _load_data(self):
        if not os.path.exists("save_data.json"): return
        try:
