...
def build_level(level_id=1, difficulty=1.0):
    ...
    if level_id == 1:
        ...
        platforms.append(Platform(2280, 590, 1680, 40, 0))
        ...
        # Expansão v2.0
        enemies.append(Enemy(2800, 550, 100, hp(4)))
        enemies.append(FlyingEnemy(3000, 450, hp(3), float_height=350))
        enemies.append(Enemy(3200, 550, 100, hp(4)))
        platforms.append(Platform(3000, 480, 200, 24, 2))
        enemies.append(ShootingEnemy(3050, 440, 80, hp(3)))

        checkpoints.append(Checkpoint(2000, 424))
        checkpoints.append(Checkpoint(3000, 544))
        orbs.append(HealthOrb(2100, 380))
        orbs.append(HealthOrb(3100, 430))
    ...
    # ── LEVEL 2: Plataformas — corrigido, sem espinhos duplos ─
        ...
        platforms.append(Platform(2980, 580, 1200, 40, 0))
        ...
        # Expansão v2.0
        enemies.append(ShootingEnemy(3400, 540, 100, hp(4)))
        enemies.append(FlyingEnemy(3600, 450, hp(3), float_height=350))
        m_plats.append(MovingPlatform(3800, 520, 140, 22, 0, -200))
        platforms.append(Platform(4000, 600, 500, 40, 1))
        enemies.append(Enemy(4100, 560, 80, hp(5)))

        orbs.append(HealthOrb(2680, 470))
        orbs.append(HealthOrb(3700, 450))
        checkpoints.append(Checkpoint(2700, 516))
        checkpoints.append(Checkpoint(3800, 556))
    ...
    # ── LEVEL 3: Puzzles — switches, portões, espinhos alternados ─
        ...
        switches.append(Switch(600, 528)) # v2 spec
        gates.append(Gate(1100, 340, 40, 220)) # v2 spec
        ...
        platforms.append(Platform(1130, 560, 350, 30, 2))
        # Dois espinhos com offsets bem separados (nunca ativos juntos)
        platforms.append(Platform(1500, 540, 150, 28, 1, hazard=True, hazard_timed=True, hazard_offset=0))
        platforms.append(Platform(1700, 540, 150, 28, 1, hazard=True, hazard_timed=True, hazard_offset=60))
        platforms.append(Platform(1900, 600, 300, 40, 0))

        enemies.append(ShootingEnemy(1200, 520, 80, hp(3)))
        enemies.append(Enemy(1300, 520, 60, hp(4)))
        enemies.append(ShootingEnemy(1550, 500, 60, hp(3)))
        enemies.append(FlyingEnemy(1700, 430, hp(2), float_height=380))
        enemies.append(Enemy(1950, 560, 80, hp(4)))

        orbs.append(HealthOrb(1800, 470))
        checkpoints.append(Checkpoint(1200, 516))

        # Zona 3: Moving platforms + combinação
        m_plats.append(MovingPlatform(2250, 540, 140, 22, 0, -160))
        platforms.append(Platform(2450, 580, 320, 40, 0))
        m_plats.append(MovingPlatform(2830, 560, 130, 22, 180, 0))
        platforms.append(Platform(3060, 560, 300, 30, 2))
        platforms.append(Platform(3420, 600, 1380, 40, 0))

        enemies.append(Enemy(2300, 500, 80, hp(4)))
        enemies.append(ShootingEnemy(2480, 540, 80, hp(3)))
        enemies.append(FlyingEnemy(2700, 430, hp(3), float_height=370))
        enemies.append(Enemy(2880, 520, 70, hp(4)))
        enemies.append(Enemy(3100, 520, 80, hp(4)))
        enemies.append(ShootingEnemy(3200, 520, 70, hp(3)))
        enemies.append(Enemy(3460, 560, 120, hp(4)))
        enemies.append(FlyingEnemy(3600, 480, hp(2), float_height=400))
        enemies.append(Enemy(3700, 560, 100, hp(4)))

        # Expansão v2.0
        enemies.append(ShootingEnemy(4000, 560, 80, hp(4)))
        enemies.append(Enemy(4200, 560, 100, hp(5)))
        platforms.append(Platform(4100, 480, 200, 24, 1))
        enemies.append(FlyingEnemy(4300, 400, hp(3), float_height=320))

        orbs.append(HealthOrb(3100, 490))
        orbs.append(HealthOrb(4100, 430))
        checkpoints.append(Checkpoint(3170, 536))
        checkpoints.append(Checkpoint(4100, 556))
    ...
    # ── LEVEL 4: A Ascensão — desafio de pulo vertical ───────
        ...
        # Expansão v2.0
        platforms.append(Platform(4000, 500, 1100, 40, 1))
        enemies.append(Enemy(4100, 460, 120, hp(5)))
        enemies.append(ShootingEnemy(4300, 460, 100, hp(5)))
        enemies.append(FlyingEnemy(4500, 350, hp(4), float_height=300))
        enemies.append(Enemy(4700, 460, 120, hp(5)))
        enemies.append(ShootingEnemy(4900, 460, 100, hp(5)))

        checkpoints.append(Checkpoint(3100, 526))
        checkpoints.append(Checkpoint(4200, 476))
        orbs.append(HealthOrb(3400, 470))
        orbs.append(HealthOrb(4600, 420))
    ...
    # ── LEVEL 5: O Boss Final — arena corrigida ───────────────
        ...
        gates.append(Gate(ARENA_X2, 0, 40, 900))             # Boss Gate (parede direita)
...
class Game:
    ...
    def update(self):
        ...
        if self.current_level == 5 and self.hud.boss_ref:
            boss = self.hud.boss_ref
            dead_state = getattr(boss, 'DEAD', 'dead')
            if getattr(boss, 'state', '') == dead_state and getattr(boss, 'dead_timer', 0) >= 40:
                sw_active = True
                ...
                if event.type==pygame.KEYDOWN:
                    ...
                    if self.state==self.S_START:
                        ...
                        if event.key==pygame.K_k:
                            self.unlocked=1; self.souls=0; self.health_upgrades=0
