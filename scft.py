...
def build_level(level_id=1, difficulty=1.0):
    ...
    if level_id == 1:
        ...
        # Expansão L1
        platforms.append(Platform(2700, 560, 600, 40, 1))
        platforms.append(Platform(3400, 560, 500, 40, 0))
        enemies.append(Enemy(2800, 520, 100, hp(4)))
        enemies.append(Enemy(3100, 520, 100, hp(4)))
        enemies.append(Enemy(3500, 520, 100, hp(5)))
        enemies.append(FlyingEnemy(3300, 480, hp(3), float_height=400))
        orbs.append(HealthOrb(3000, 520))
    ...
    # ── LEVEL 2: Plataformas — corrigido, sem espinhos duplos ─
        ...
        # Expansão L2
        platforms.append(Platform(2950, 600, 700, 40, 0))
        m_plats.append(MovingPlatform(3700, 600, 150, 22, 400, 0))
        platforms.append(Platform(4150, 600, 300, 40, 1))
        enemies.append(ShootingEnemy(3100, 560, 80, hp(4)))
        enemies.append(Enemy(3300, 560, 100, hp(5)))
        enemies.append(ShootingEnemy(3500, 560, 80, hp(4)))
        enemies.append(FlyingEnemy(3900, 500, hp(3), float_height=420))
        orbs.append(HealthOrb(4250, 560))
    ...
    # ── LEVEL 3: Puzzles — switches, portões, espinhos alternados ─
        ...
        # Expansão L3
        platforms.append(Platform(3000, 580, 500, 40, 0))
        platforms.append(Platform(3600, 580, 500, 40, 1))
        platforms.append(Platform(4200, 580, 550, 40, 0))
        enemies.append(Enemy(3100, 540, 120, hp(5)))
        enemies.append(ShootingEnemy(3300, 540, 100, hp(4)))
        enemies.append(Enemy(3700, 540, 120, hp(5)))
        enemies.append(ShootingEnemy(3900, 540, 100, hp(4)))
        enemies.append(Enemy(4300, 540, 120, hp(5)))
    ...
    # ── LEVEL 4: A Ascensão — desafio de pulo vertical ───────
        ...
        # Expansão L4
        platforms.append(Platform(4000, 550, 500, 40, 0))
        platforms.append(Platform(4600, 550, 550, 40, 1))
        enemies.append(Enemy(4100, 510, 120, hp(5)))
        enemies.append(ShootingEnemy(4300, 510, 100, hp(5)))
        enemies.append(Enemy(4700, 510, 120, hp(6)))
        enemies.append(ShootingEnemy(4900, 510, 100, hp(5)))
        enemies.append(FlyingEnemy(4500, 400, hp(4), float_height=320))
    ...
    # ── LEVEL 5: O Boss Final — arena corrigida ───────────────
        ...
        # Portão do Boss (substitui a parede sólida direita)
...
class Game:
    ...
    def update(self):
        ...
        if self.state==self.S_SELECT:
            if keys[pygame.K_k]:
                ...
                if self.reset_timer<120:
                    self.hud.show_message("Segure K para resetar...", 2)
                else:
                    self.unlocked=1; self.souls=0; self.health_upgrades=0; self.difficulty=1.0
                    self._save_data(); self.reset_timer=0
                    self.hud.show_message("Progresso Resetado!", 60)
            else:
                self.reset_timer=0
        ...
        # Portão do Boss Level 5
        if self.current_level==5 and self.hud.boss_ref:
            b=self.hud.boss_ref
            if (getattr(b,'state','') in ('dead',Enemy.DEAD)) and getattr(b,'dead_timer',0)>=40:
                sw_active=True
