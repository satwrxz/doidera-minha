...
class HUD:
    ...
    def draw_reset_ui(self, surf, reset_timer, reset_msg_timer):
        if reset_timer > 0:
            bw, bh = 200, 10
            bx, by = SCREEN_W // 2 - bw // 2, 660
            pygame.draw.rect(surf, (40, 40, 40), (bx, by, bw, bh))
            pygame.draw.rect(surf, (200, 200, 200), (bx, by, int(bw * (reset_timer / 120)), bh))
            msg = self.font_sm.render("Segure K para resetar...", True, (255, 255, 255))
            surf.blit(msg, (SCREEN_W // 2 - msg.get_width() // 2, by - 20))

        if reset_msg_timer > 0:
            msg = self.font_md.render("Progresso Resetado!", True, (100, 255, 100))
            surf.blit(msg, (SCREEN_W // 2 - msg.get_width() // 2, 660))
                ...
                if event.type==pygame.KEYDOWN:
                    if event.key==pygame.K_ESCAPE:
                        if self.state==self.S_PLAY:
                            self.state=self.S_SELECT; self._save_data()
                        else: running=False
