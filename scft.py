        ...
        if self.on_ground and self.state in (self.PATROL, self.CHASE, self.ATTACK):
            ...
            if on_edge:
                if self.state == self.PATROL:
                    ...
                    # Tenta pular gap em perseguição/ataque
...
class HUD:
    ...
    def draw_start_screen(self,surf):
        ...
        draw_rect_alpha(surf,C_CHK_GLOW,
            (SCREEN_W//2-title.get_width()//2-12,int(SCREEN_H//2-68 + float_y),
             title.get_width()+24,72),gl//4)
        surf.blit(title,(SCREEN_W//2-title.get_width()//2,int(SCREEN_H//2-62 + float_y)))
...
def build_level(level_id=1, difficulty=1.0):
    ...
    if level_id == 1:
        ...
        # Expansão v2.0
        platforms.append(Platform(3000, 560, 400, 40, 0))
        platforms.append(Platform(3500, 560, 450, 40, 1))
        enemies.append(Enemy(3100, 520, 100, hp(4)))
        enemies.append(ShootingEnemy(3300, 520, 80, hp(3)))
        enemies.append(Enemy(3600, 520, 120, hp(4)))
    ...
    # ── LEVEL 2: Plataformas — corrigido, sem espinhos duplos ─
        ...
        # Expansão v2.0
        platforms.append(Platform(3400, 580, 500, 40, 0))
        platforms.append(Platform(3950, 600, 450, 40, 1))
        enemies.append(Enemy(3500, 540, 100, hp(4)))
        enemies.append(FlyingEnemy(3700, 480, hp(3), float_height=420))
        enemies.append(ShootingEnemy(4050, 560, 80, hp(3)))
        enemies.append(Enemy(4200, 560, 120, hp(4)))
    ...
    # ── LEVEL 3: Puzzles — switches, portões, espinhos alternados ─
        ...
        # Expansão v2.0
        platforms.append(Platform(3600, 580, 500, 40, 0))
        platforms.append(Platform(4150, 600, 400, 40, 1))
        enemies.append(Enemy(3700, 540, 100, hp(4)))
        enemies.append(FlyingEnemy(3900, 480, hp(3), float_height=420))
        enemies.append(ShootingEnemy(4250, 560, 80, hp(3)))
        enemies.append(Enemy(4400, 560, 120, hp(5)))
    ...
    # ── LEVEL 4: A Ascensão — desafio de pulo vertical ───────
        ...
        platforms.append(Platform(4500, 560, 500, 40, 1))
        enemies.append(Enemy(4100, 520, 100, hp(5)))
        enemies.append(FlyingEnemy(4300, 460, hp(4), float_height=400))
        enemies.append(ShootingEnemy(4600, 520, 80, hp(4)))
        enemies.append(Enemy(4800, 520, 120, hp(5)))
...
class Game:
    ...
    def _load_data(self):
        ...
        except (json.JSONDecodeError, IOError, OSError, FileNotFoundError): pass
    ...
    def update(self):
        ...
        if self.hud.boss_ref:
            b = self.hud.boss_ref
            b_dead = getattr(b, 'state', '') in (getattr(Enemy, 'DEAD', 'dead'), 'DEAD')
            if b_dead and getattr(b, 'dead_timer', 0) >= 40:
                sw_active = True
