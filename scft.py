...
def build_level(level_id=1, difficulty=1.0):
    ...
    if level_id == 1:
        ...
        platforms.append(Platform(3100, 550, 200, 24, 2))
        platforms.append(Platform(3400, 520, 200, 24, 0))
        platforms.append(Platform(3700, 550, 300, 40, 1))
        enemies.append(Enemy(3150, 510, 60, hp(4)))
        enemies.append(ShootingEnemy(3450, 480, 50, hp(3)))
    ...
    # ── LEVEL 2: Plataformas — corrigido, sem espinhos duplos ─
        ...
        # Zona 4: Expansão v2.0
        platforms.append(Platform(3500, 580, 200, 30, 0))
        m_plats.append(MovingPlatform(3750, 540, 130, 22, 0, -150))
        platforms.append(Platform(4000, 540, 200, 30, 2))
        platforms.append(Platform(4250, 590, 250, 40, 1))

        enemies.append(Enemy(3550, 540, 60, hp(4)))
        enemies.append(ShootingEnemy(4050, 500, 60, hp(3)))
        enemies.append(FlyingEnemy(4150, 420, hp(3), float_height=340))
    ...
    # ── LEVEL 3: Puzzles — switches, portões, espinhos alternados ─
        ...
        # Zona 1: Intro + switch (Posições v2.0: Switch x=600, Gate x=1100)
        ...
        # Zona 4: Expansão v2.0
        platforms.append(Platform(3700, 560, 200, 30, 1))
        platforms.append(Platform(4000, 520, 200, 30, 2))
        platforms.append(Platform(4300, 570, 200, 30, 0))
        platforms.append(Platform(4550, 570, 250, 40, 1))

        enemies.append(Enemy(3750, 520, 60, hp(5)))
        enemies.append(ShootingEnemy(4050, 480, 60, hp(4)))
        enemies.append(FlyingEnemy(4200, 400, hp(4), float_height=320))
    ...
    # ── LEVEL 4: A Ascensão — desafio de pulo vertical ───────
        ...
        # Zona 4: Expansão v2.0
        platforms.append(Platform(4100, 520, 200, 24, 2))
        platforms.append(Platform(4400, 480, 200, 24, 0))
        platforms.append(Platform(4700, 520, 200, 24, 1))
        platforms.append(Platform(4950, 540, 250, 40, 0))

        enemies.append(Enemy(4150, 480, 60, hp(5)))
        enemies.append(ShootingEnemy(4450, 440, 60, hp(5)))
        enemies.append(FlyingEnemy(4600, 380, hp(4), float_height=300))
    ...
    # ── LEVEL 5: O Boss Final — arena corrigida ───────────────
        ...
        gates.append(Gate(ARENA_X2, 0, 40, 600))             # Portão do Boss

        # Logic for Boss Gate (uses existing switch logic but tied to boss death)
...
class Game:
    ...
    def update(self):
        ...
        # Switches / Gates
        boss_dead = self.current_level==5 and (not self.hud.boss_ref or not self.hud.boss_ref.alive)
