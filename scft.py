        ...
        if self.on_ground and self.state in (self.PATROL, self.CHASE, self.ATTACK):
            ...
            if on_edge:
                if self.state in (self.CHASE, self.ATTACK):
                    # Tenta pular o buraco se estiver perseguindo/atacando
...
class HUD:
    ...
    def draw_start_screen(self,surf):
        ...
        # Floating animation: freq 2.0, amp 10
...
def build_level(level_id=1, difficulty=1.0):
    ...
    if level_id == 1:
        ...
        # Zona 4: Expansão v2 (x > 2700)
        platforms.append(Platform(3000, 540, 300, 30, 1))
        platforms.append(Platform(3350, 600, 650, 40, 0))
        enemies.append(Enemy(3100, 500, 80, hp(4)))
        enemies.append(FlyingEnemy(3400, 500, hp(2), float_height=420))
        enemies.append(Enemy(3600, 560, 100, hp(4)))
    ...
    # ── LEVEL 2: Plataformas — corrigido, sem espinhos duplos ─
        ...
        # Zona 4: Expansão v2 (x > 3200)
        m_plats.append(MovingPlatform(3400, 500, 150, 22, 0, -200))
        platforms.append(Platform(3600, 300, 200, 30, 1))
        platforms.append(Platform(3900, 580, 600, 40, 0))
        enemies.append(ShootingEnemy(3650, 260, 80, hp(4)))
        enemies.append(Enemy(4000, 540, 120, hp(5)))
        enemies.append(FlyingEnemy(4200, 460, hp(3), float_height=400))
    ...
    # ── LEVEL 3: Puzzles — switches, portões, espinhos alternados ─
        ...
        switches.append(Switch(600, 584))
        gates.append(Gate(1100, 340, 40, 300))
        ...
        platforms.append(Platform(730, 620, 350, 30, 2))
        # Dois espinhos com offsets bem separados (nunca ativos juntos)
        platforms.append(Platform(1200, 600, 150, 28, 1, hazard=True, hazard_timed=True, hazard_offset=0))
        platforms.append(Platform(1400, 600, 150, 28, 1, hazard=True, hazard_timed=True, hazard_offset=60))
        platforms.append(Platform(1600, 640, 300, 40, 0))

        enemies.append(ShootingEnemy(800, 580, 80, hp(3)))
        enemies.append(Enemy(900, 580, 60, hp(4)))
        enemies.append(ShootingEnemy(1250, 560, 60, hp(3)))
        enemies.append(FlyingEnemy(1400, 490, hp(2), float_height=440))
        enemies.append(Enemy(1650, 600, 80, hp(4)))

        orbs.append(HealthOrb(1400, 470))
        checkpoints.append(Checkpoint(1200, 566))

        # Zona 3: Moving platforms + combinação
        m_plats.append(MovingPlatform(2000, 580, 140, 22, 0, -160))
        platforms.append(Platform(2200, 620, 320, 40, 0))
        m_plats.append(MovingPlatform(2600, 600, 130, 22, 180, 0))
        platforms.append(Platform(2800, 600, 300, 30, 2))
        platforms.append(Platform(3200, 640, 540, 40, 0))

        enemies.append(Enemy(2050, 540, 80, hp(4)))
        enemies.append(ShootingEnemy(2230, 580, 80, hp(3)))
        enemies.append(FlyingEnemy(2450, 470, hp(3), float_height=410))
        enemies.append(Enemy(2630, 560, 70, hp(4)))
        enemies.append(Enemy(2850, 560, 80, hp(4)))
        enemies.append(ShootingEnemy(3000, 560, 70, hp(3)))

        # Zona 4: Expansão v2 (x > 3600)
        platforms.append(Platform(3800, 500, 300, 30, 1))
        m_plats.append(MovingPlatform(4200, 580, 150, 22, 100, -100))
        platforms.append(Platform(4400, 580, 400, 40, 0))
        enemies.append(Enemy(3850, 460, 100, hp(5)))
        enemies.append(ShootingEnemy(3950, 460, 80, hp(4)))
        enemies.append(FlyingEnemy(4500, 480, hp(3), float_height=420))

        orbs.append(HealthOrb(2850, 530))
        checkpoints.append(Checkpoint(3250, 596))
    ...
    # ── LEVEL 4: A Ascensão — desafio de pulo vertical ───────
        ...
        # Zona final: Expansão v2 (x > 4000)
        platforms.append(Platform(4000, 450, 300, 30, 2))
        platforms.append(Platform(4400, 550, 750, 40, 0))
        enemies.append(Enemy(4100, 410, 80, hp(6)))
        enemies.append(ShootingEnemy(4450, 510, 100, hp(5)))
        enemies.append(Enemy(4600, 510, 120, hp(6)))
        enemies.append(FlyingEnemy(4800, 450, hp(4), float_height=380))
    ...
    # ── LEVEL 5: O Boss Final — arena corrigida ───────────────
        ...
        # ARENA_X2 será a porta (Gate) que abre ao derrotar o boss
        gates.append(Gate(ARENA_X2, 0, 40, 560))
...
class Game:
    ...
    def update(self):
        ...
        # Boss Gate Logic (Level 5)
        boss_dead = False
        if self.current_level == 5 and self.hud.boss_ref:
            b = self.hud.boss_ref
            # Abre o portão após animação de morte (40 frames)
            if getattr(b, 'state', '') == getattr(Enemy, 'DEAD', 'dead') and getattr(b, 'dead_timer', 0) >= 40:
                boss_dead = True

        for g in self.gates:
            g.update(sw_active or boss_dead)
