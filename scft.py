...
class Enemy:
    ...
    def update(self, platforms, player_rect):
        ...
        # Borda de plataforma: PATROL inverte, CHASE/ATTACK pula
        if self.on_ground and self.state in (self.PATROL, self.CHASE, self.ATTACK):
            ...
            if on_edge:
                if self.state == self.PATROL:
                    ...
                    # Tenta pular o buraco se estiver agressivo
...
def build_level(level_id=1, difficulty=1.0):
    ...
    if level_id == 1:
        ...
        # ZONA EXPANDIDA v2.0
        platforms.append(Platform(3000, 540, 200, 30, 1))
        platforms.append(Platform(3250, 480, 180, 24, 2))
        platforms.append(Platform(3500, 560, 400, 40, 0))
        enemies.append(Enemy(3050, 500, 70, hp(3)))
        enemies.append(ShootingEnemy(3550, 520, 80, hp(3)))
        enemies.append(FlyingEnemy(3300, 400, hp(2), float_height=350))
    ...
    # ── LEVEL 2: Plataformas — corrigido, sem espinhos duplos ─
        ...
        # ZONA EXPANDIDA v2.0
        m_plats.append(MovingPlatform(3400, 520, 150, 22, 0, -200))
        platforms.append(Platform(3650, 480, 250, 30, 1))
        m_plats.append(MovingPlatform(3950, 560, 140, 22, 150, 0))
        platforms.append(Platform(4200, 600, 250, 40, 0))

        enemies.append(ShootingEnemy(3700, 440, 60, hp(3)))
        enemies.append(FlyingEnemy(4000, 400, hp(2), float_height=320))
        enemies.append(Enemy(4250, 560, 80, hp(5)))

        orbs.append(HealthOrb(3700, 410))
        checkpoints.append(Checkpoint(3650, 456))
    ...
    # ── LEVEL 3: Puzzles — switches, portões, espinhos alternados ─
        ...
        # Zona 1: Intro + switch (REPOSICIONADO v2.0)
        ...
        # ZONA EXPANDIDA v2.0
        platforms.append(Platform(3700, 540, 250, 30, 1, hazard=True, hazard_timed=True))
        m_plats.append(MovingPlatform(4000, 500, 140, 22, 0, -150))
        platforms.append(Platform(4200, 580, 450, 40, 0))

        enemies.append(ShootingEnemy(3750, 500, 70, hp(4)))
        enemies.append(Enemy(4250, 540, 100, hp(5)))
        enemies.append(FlyingEnemy(4100, 400, hp(3), float_height=320))

        orbs.append(HealthOrb(3800, 470))
        checkpoints.append(Checkpoint(4200, 536))
    ...
    # ── LEVEL 4: A Ascensão — desafio de pulo vertical ───────
        ...
        # ZONA EXPANDIDA v2.0
        m_plats.append(MovingPlatform(4000, 520, 150, 22, 250, 0))
        platforms.append(Platform(4300, 560, 200, 30, 2))
        platforms.append(Platform(4550, 500, 180, 26, 1, hazard=True, hazard_timed=True))
        platforms.append(Platform(4800, 540, 350, 40, 0))

        enemies.append(ShootingEnemy(4350, 520, 80, hp(5)))
        enemies.append(Enemy(4850, 500, 100, hp(6)))
        enemies.append(FlyingEnemy(4600, 350, hp(4), float_height=280))

        checkpoints.append(Checkpoint(4300, 516))
        orbs.append(HealthOrb(4600, 430))
    ...
    # ── LEVEL 5: O Boss Final — arena corrigida ───────────────
        ...
        # ── ARENA DO BOSS ─────────────────────────────────────
        # Chão sólido e extenso — garante que o boss não caia (EXTENDIDO v2.0)
        ...
        # GATE DO BOSS v2.0 (Barreira que abre ao derrotar boss)
        gates.append(Gate(ARENA_X2, 0, 40, 600))
...
class Game:
    ...
    def update(self):
        ...
        # Switches / Gates
        boss_defeated = (self.current_level == 5 and self.hud.boss_ref and
                         self.hud.boss_ref.state == Enemy.DEAD)
        gate_active = any(s.active for s in self.switches) or boss_defeated
        for s in self.switches: s.update(self.player, self.player.particles)
        for g in self.gates:    g.update(gate_active)
