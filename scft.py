...
class Enemy:
    ...
    def update(self, platforms, player_rect):
        ...
        # Borda de plataforma: NUNCA pula se PATROL, só vira — corrige o bug de cair
        if self.on_ground and self.state in (self.PATROL, self.CHASE, self.ATTACK):
            ...
            if on_edge:
                if self.state == self.PATROL:
                    ...
                    # Tenta pular se estiver em perseguição/ataque
...
def build_level(level_id=1, difficulty=1.0):
    ...
    # ── LEVEL 5: O Boss Final — arena corrigida ───────────────
        ...
        # Chão estendido além de ARENA_X2 por 400px
...
class Game:
    ...
    def update(self):
        ...
        if self.current_level == 5 and self.hud.boss_ref:
            boss_dead = self.hud.boss_ref.state == Enemy.DEAD
            boss_dead_timer = self.hud.boss_ref.dead_timer
            gate_active = boss_dead and boss_dead_timer >= 40
