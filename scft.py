...
class Enemy:
    ...
    def update(self, platforms, player_rect):
        ...
        # Borda de plataforma: PATROL vira, CHASE/ATTACK tenta pular
...
def build_level(level_id=1, difficulty=1.0):
    ...
    if level_id == 1:
        ...
        platforms.append(Platform(3000, 500, 300, 30, 2))
        platforms.append(Platform(3400, 560, 500, 40, 0))
        enemies.append(Enemy(3100, 460, 80, hp(4)))
        enemies.append(FlyingEnemy(3300, 420, hp(3), float_height=380))
        enemies.append(Enemy(3500, 520, 100, hp(4)))
        enemies.append(Enemy(3700, 520, 100, hp(4)))

        checkpoints.append(Checkpoint(3000, 456))
        orbs.append(HealthOrb(3500, 480))
    ...
    # ── LEVEL 2: Plataformas — corrigido, sem espinhos duplos ─
        ...
        # Zona 4: Expansão v2.0
        platforms.append(Platform(3450, 500, 300, 30, 2))
        m_plats.append(MovingPlatform(3800, 540, 140, 22, 0, -180))
        platforms.append(Platform(4000, 560, 400, 40, 0))
        enemies.append(Enemy(3500, 460, 80, hp(4)))
        enemies.append(ShootingEnemy(4100, 520, 80, hp(4)))
        enemies.append(Enemy(4250, 520, 80, hp(4)))
        enemies.append(FlyingEnemy(3900, 400, hp(3), float_height=350))

        orbs.append(HealthOrb(3500, 430))
        checkpoints.append(Checkpoint(3450, 456))
    ...
    # ── LEVEL 3: Puzzles — switches, portões, espinhos alternados ─
        ...
        platforms.append(Platform(550, 560, 300, 30, 1))
        ...
        m_plats.append(MovingPlatform(2400, 540, 140, 22, 0, -160))
        platforms.append(Platform(2600, 580, 320, 40, 0))
        m_plats.append(MovingPlatform(3000, 560, 130, 22, 180, 0))
        platforms.append(Platform(3250, 560, 300, 30, 2))
        platforms.append(Platform(3650, 600, 540, 40, 0))

        enemies.append(Enemy(2450, 500, 80, hp(4)))
        enemies.append(ShootingEnemy(2630, 540, 80, hp(3)))
        enemies.append(FlyingEnemy(2900, 430, hp(3), float_height=370))
        enemies.append(Enemy(3100, 520, 70, hp(4)))
        enemies.append(Enemy(3300, 520, 80, hp(4)))
        enemies.append(ShootingEnemy(3450, 520, 70, hp(3)))
        enemies.append(Enemy(3750, 560, 120, hp(4)))

        # Zona 4: Expansão v2.0
        platforms.append(Platform(4250, 520, 250, 30, 1))
        enemies.append(Enemy(4300, 480, 60, hp(4)))
        enemies.append(FlyingEnemy(4500, 400, hp(3), float_height=350))
        enemies.append(ShootingEnemy(4600, 480, 50, hp(4)))

        orbs.append(HealthOrb(3300, 490))
        checkpoints.append(Checkpoint(3300, 536))

        # Goal final extendido
        platforms.append(Platform(4550, 600, 250, 40, 0))
        goal = LevelGoal(4700, 524)
    ...
    # ── LEVEL 4: A Ascensão — desafio de pulo vertical ───────
        ...
        # Zona final expandida v2.0
        platforms.append(Platform(4000, 450, 300, 30, 2))
        m_plats.append(MovingPlatform(4400, 500, 150, 22, 300, 0))
        platforms.append(Platform(4800, 550, 350, 40, 0))
        enemies.append(Enemy(4100, 410, 80, hp(5)))
        enemies.append(FlyingEnemy(4500, 350, hp(3), float_height=300))
        enemies.append(ShootingEnemy(4900, 510, 80, hp(5)))

        goal = LevelGoal(5080, 474)
    ...
    # ── LEVEL 5: O Boss Final — arena corrigida ───────────────
        ...
        platforms.append(Platform(ARENA_X2+400, 0, 40, 900))   # parede direita arena
...
class Game:
    ...
    def _load_data(self):
        try:
            ...
            print(f"Erro ao carregar (novo jogo?): {e}")
    ...
    def update(self):
        ...
        if self.state==self.S_PLAY and self.entry_fade>0:
