...
def build_level(level_id=1, difficulty=1.0):
    ...
    if level_id == 1:
        ...
        # Expansão Level 1
        platforms.append(Platform(2960, 550, 400, 40, 1))
        platforms.append(Platform(3400, 520, 500, 40, 0))
        enemies.append(Enemy(3100, 510, 80, hp(4)))
        enemies.append(ShootingEnemy(3500, 480, 80, hp(3)))
        enemies.append(Enemy(3700, 480, 80, hp(4)))
        enemies.append(FlyingEnemy(3300, 420, hp(2), float_height=350))
    ...
    # ── LEVEL 2: Plataformas — corrigido, sem espinhos duplos ─
        ...
        # Expansão Level 2
        platforms.append(Platform(3400, 550, 400, 30, 1))
        m_plats.append(MovingPlatform(3850, 550, 140, 22, 200, 0))
        platforms.append(Platform(4100, 560, 400, 40, 0))
        enemies.append(ShootingEnemy(3450, 510, 80, hp(4)))
        enemies.append(Enemy(3600, 510, 70, hp(4)))
        enemies.append(FlyingEnemy(3900, 420, hp(3), float_height=360))
        enemies.append(Enemy(4200, 520, 100, hp(5)))
    ...
    # ── LEVEL 3: Puzzles — switches, portões, espinhos alternados ─
        ...
        # Expansão Level 3
        platforms.append(Platform(3600, 580, 400, 35, 1))
        m_plats.append(MovingPlatform(4100, 540, 150, 22, 250, 0))
        platforms.append(Platform(4400, 550, 400, 40, 0))
        enemies.append(Enemy(3650, 540, 80, hp(5)))
        enemies.append(ShootingEnemy(3750, 540, 80, hp(4)))
        enemies.append(FlyingEnemy(4150, 430, hp(3), float_height=370))
        enemies.append(Enemy(4500, 510, 100, hp(5)))
    ...
    # ── LEVEL 4: A Ascensão — desafio de pulo vertical ───────
        ...
        # Expansão Level 4
        platforms.append(Platform(4000, 520, 400, 30, 1))
        m_plats.append(MovingPlatform(4450, 500, 140, 22, 200, 0))
        platforms.append(Platform(4700, 510, 400, 40, 0))
        enemies.append(ShootingEnemy(4100, 480, 80, hp(5)))
        enemies.append(Enemy(4250, 480, 80, hp(5)))
        enemies.append(FlyingEnemy(4500, 400, hp(4), float_height=340))
        enemies.append(Enemy(4800, 470, 100, hp(6)))
...
class Game:
    ...
    def update(self):
        ...
        if self.current_level == 5 and self.hud.boss_ref:
            ...
            # Gate opens if boss is dead for more than 40 frames
            if getattr(boss, 'state', '') == getattr(Enemy, 'DEAD', 'dead') and getattr(boss, 'dead_timer', 0) > 40:
                sw_active = True
