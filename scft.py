...
class HUD:
    ...
    def draw_start_screen(self,surf):
        ...
        y_float = int(math.sin(pygame.time.get_ticks() * 0.002 * 2.0) * 10)
        title=ft.render("SHADOWCROFT",True,C_UI_TITLE)
        draw_rect_alpha(surf,C_CHK_GLOW,
            (SCREEN_W//2-title.get_width()//2-12,SCREEN_H//2-68 + y_float,
             title.get_width()+24,72),gl//4)
        surf.blit(title,(SCREEN_W//2-title.get_width()//2,SCREEN_H//2-62 + y_float))
    ...
    def draw_level_select(self,surf,unlocked,souls,health_upgrades):
        ...
        hint=fs.render("1-5 Fases | ESC sair | E/N/H Dificuldade | K Reset",True,(100,95,130))
...
def build_level(level_id=1, difficulty=1.0):
    ...
    if level_id == 1:
        ...
        platforms.append(Platform(3400, 500, 200, 30, 2))
        enemies.append(Enemy(3150, 510, 60, hp(4)))
        enemies.append(FlyingEnemy(3450, 400, hp(2), float_height=350))
    ...
    # ── LEVEL 2: Plataformas — corrigido, sem espinhos duplos ─
        ...
        # Expanded area L2
        platforms.append(Platform(3100, 550, 200, 30, 1))
        platforms.append(Platform(3400, 500, 220, 30, 0))
        platforms.append(Platform(3700, 560, 200, 30, 2))
        enemies.append(Enemy(3450, 460, 70, hp(4)))
        enemies.append(ShootingEnemy(3750, 520, 60, hp(3)))
    ...
    # ── LEVEL 3: Puzzles — switches, portões, espinhos alternados ─
        ...
        platforms.append(Platform(3100, 550, 200, 30, 1))
        platforms.append(Platform(3400, 500, 200, 30, 2))
        platforms.append(Platform(3700, 450, 200, 30, 0))
        enemies.append(Enemy(3750, 410, 60, hp(5)))
        enemies.append(FlyingEnemy(3450, 400, hp(3), float_height=350))
    ...
    # ── LEVEL 4: A Ascensão — desafio de pulo vertical ───────
        ...
        # Expanded area L4
        platforms.append(Platform(4100, 500, 300, 30, 0))
        platforms.append(Platform(4500, 450, 200, 30, 2))
        enemies.append(ShootingEnemy(4150, 460, 80, hp(5)))
        enemies.append(Enemy(4550, 410, 60, hp(5)))
    ...
    # ── LEVEL 5: O Boss Final — arena corrigida ───────────────
        ...
        # Boss Gate (replaces static wall at ARENA_X2)
...
class Game:
    ...
    def update(self):
        ...
        # Boss Gate Level 5 activation logic
        if self.current_level == 5 and self.hud.boss_ref:
            b = self.hud.boss_ref
            # Gate opens after boss death animation
            if getattr(b, 'state', '') in ('DEAD', 'dead') and getattr(b, 'dead_timer', 0) >= 40:
                sw_active = True
                ...
                if event.type==pygame.KEYDOWN:
                    ...
                    if self.state==self.S_START:
                        ...
                        if event.key==pygame.K_k:
                            ...
                            self._save_data(); self._init_game()
