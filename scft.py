...
def build_level(level_id=1, difficulty=1.0):
    ...
    if level_id == 1:
        ...
        # Zona 4: Expansão v2.0
        platforms.append(Platform(3000, 520, 200, 24, 2))
        platforms.append(Platform(3300, 600, 600, 40, 1))
        enemies.append(Enemy(3350, 560, 100, hp(4)))
        enemies.append(ShootingEnemy(3550, 560, 80, hp(3)))
        enemies.append(FlyingEnemy(3700, 450, hp(2), float_height=390))
    ...
    # ── LEVEL 2: Plataformas — corrigido, sem espinhos duplos ─
        ...
        # Zona 4: Expansão v2.0
        m_plats.append(MovingPlatform(3400, 540, 130, 22, 0, -200))
        platforms.append(Platform(3600, 380, 300, 30, 1))
        enemies.append(ShootingEnemy(3650, 340, 80, hp(4)))
        enemies.append(FlyingEnemy(3800, 300, hp(2), float_height=240))
        platforms.append(Platform(3950, 580, 500, 40, 0))
        enemies.append(Enemy(4050, 540, 100, hp(5)))
    ...
    # ── LEVEL 3: Puzzles — switches, portões, espinhos alternados ─
        ...
        # Zona 1: Intro + switch (REDIRECIONADO v2.0)
        platforms.append(Platform(  0, 620, 500, 40, 0))
        switches.append(Switch(600, 580))
        gates.append(Gate(1100, 400, 40, 240))
        platforms.append(Platform(500, 620, 500, 40, 1))
        platforms.append(Platform(1000, 640, 400, 40, 0))
        ...
        enemies.append(Enemy(720, 580, 60, hp(4)))
        checkpoints.append(Checkpoint(580, 576))

        # Zona 2: Após gate, espinhos alternados com offsets diferentes
        platforms.append(Platform(1200, 640, 350, 30, 2))
        # Dois espinhos com offsets bem separados (nunca ativos juntos)
        platforms.append(Platform(1600, 620, 150, 28, 1, hazard=True, hazard_timed=True, hazard_offset=0))
        platforms.append(Platform(1850, 620, 150, 28, 1, hazard=True, hazard_timed=True, hazard_offset=60))
        platforms.append(Platform(2100, 600, 300, 40, 0))

        enemies.append(ShootingEnemy(1300, 600, 80, hp(3)))
        enemies.append(Enemy(1450, 600, 60, hp(4)))
        enemies.append(ShootingEnemy(1650, 580, 60, hp(3)))
        enemies.append(FlyingEnemy(1800, 430, hp(2), float_height=380))
        enemies.append(Enemy(2150, 560, 80, hp(4)))

        orbs.append(HealthOrb(1900, 470))
        checkpoints.append(Checkpoint(1300, 596))

        # Zona 3: Moving platforms + combinação
        m_plats.append(MovingPlatform(2500, 540, 140, 22, 0, -160))
        platforms.append(Platform(2700, 580, 320, 40, 0))
        m_plats.append(MovingPlatform(3100, 560, 130, 22, 180, 0))
        platforms.append(Platform(3350, 560, 300, 30, 2))
        platforms.append(Platform(3700, 600, 540, 40, 0))

        enemies.append(Enemy(2550, 500, 80, hp(4)))
        enemies.append(ShootingEnemy(2750, 540, 80, hp(3)))
        enemies.append(FlyingEnemy(2900, 430, hp(3), float_height=370))
        enemies.append(Enemy(3150, 520, 70, hp(4)))
        enemies.append(Enemy(3400, 520, 80, hp(4)))
        enemies.append(ShootingEnemy(3550, 520, 70, hp(3)))
        enemies.append(Enemy(3750, 560, 120, hp(4)))
        enemies.append(FlyingEnemy(3900, 480, hp(2), float_height=400))

        # Zona 4: Expansão v2.0
        platforms.append(Platform(4250, 520, 200, 24, 2))
        platforms.append(Platform(4500, 580, 300, 40, 1))
        enemies.append(ShootingEnemy(4550, 540, 80, hp(5)))
        enemies.append(Enemy(4650, 540, 100, hp(5)))

        orbs.append(HealthOrb(3400, 490))
        checkpoints.append(Checkpoint(3470, 516))
    ...
    # ── LEVEL 4: A Ascensão — desafio de pulo vertical ───────
        ...
        # Zona 5: Expansão v2.0 Final
        platforms.append(Platform(4000, 480, 250, 24, 2))
        platforms.append(Platform(4350, 540, 800, 40, 1))
        enemies.append(ShootingEnemy(4400, 500, 120, hp(5)))
        enemies.append(Enemy(4600, 500, 120, hp(5)))
        enemies.append(FlyingEnemy(4800, 350, hp(4), float_height=290))
        enemies.append(ShootingEnemy(5000, 500, 100, hp(5)))
    ...
    # ── LEVEL 5: O Boss Final — arena corrigida ───────────────
        ...
        # Portão do Boss (Gate) que só abre ao derrotá-lo
        boss_gate = Gate(ARENA_X2, 0, 40, 600)
        gates.append(boss_gate)
        # Chão estendido após o portão
        platforms.append(Platform(ARENA_X2, 560, 400, 45, 0))
...
class Game:
    ...
    def _save_data(self):
        try:
            ...
            print(f"Erro de E/S ao salvar: {e}")
    ...
    def update(self):
        ...
        # Portão do Boss no Level 5 abre se o boss estiver morto
        if self.current_level == 5 and self.hud.boss_ref:
            if getattr(self.hud.boss_ref, 'state', '') == getattr(Enemy, 'DEAD', 'dead'):
                sw_active = True
