...
pygame.display.set_caption("ShadowCroft v2.0")
...
def build_level(level_id=1, difficulty=1.0):
    ...
    if level_id == 1:
        ...
        # Level expanded: add more platforms/enemies for density
        platforms.append(Platform(3000, 560, 400, 30, 1))
    ...
    # ── LEVEL 2: Plataformas — corrigido, sem espinhos duplos ─
        ...
        # Level expanded
        platforms.append(Platform(3400, 560, 400, 30, 1))
        enemies.append(Enemy(3500, 520, 100, hp(4)))
        enemies.append(ShootingEnemy(3700, 520, 80, hp(3)))
        platforms.append(Platform(3900, 580, 550, 40, 0))
    ...
    # ── LEVEL 3: Puzzles — switches, portões, espinhos alternados ─
        ...
        # Level expanded
        platforms.append(Platform(3600, 600, 400, 30, 1))
        enemies.append(Enemy(3700, 560, 100, hp(4)))
        enemies.append(ShootingEnemy(3900, 560, 80, hp(3)))
        platforms.append(Platform(4100, 540, 600, 40, 0))
    ...
    # ── LEVEL 4: A Ascensão — desafio de pulo vertical ───────
        ...
        # Level expanded
        platforms.append(Platform(4000, 560, 400, 30, 1))
        enemies.append(Enemy(4100, 520, 100, hp(5)))
        enemies.append(ShootingEnemy(4300, 520, 80, hp(4)))
        platforms.append(Platform(4500, 540, 650, 40, 0))
        enemies.append(FlyingEnemy(4700, 440, hp(3), float_height=380))
...
class Game:
    ...
    def _init_game(self):
        self.max_hp = 5 + self.health_upgrades
        ...
        self.player      = Player(80, 530, max_hp=self.max_hp)
    ...
    def update(self):
        ...
        if self.hud.boss_ref:
            b = self.hud.boss_ref
            if getattr(b, 'state', '') in (getattr(Enemy, 'DEAD', 'dead'),) and getattr(b, 'dead_timer', 0) >= 40:
                boss_dead = True
        for g in self.gates:    g.update(sw_active or boss_dead)
