        ...
        if self.on_ground and self.state in (self.PATROL, self.CHASE, self.ATTACK):
            ...
            if on_edge:
                if self.state in (self.CHASE, self.ATTACK):
                    self.vy = JUMP_FORCE * 0.75 # Pula o buraco
                else:
                    self.facing *= -1
                    self.vx = self.patrol_spd * self.facing
...
def build_level(level_id=1, difficulty=1.0):
    ...
    if level_id == 1:
        ...
        platforms.append(Platform(3350, 480, 180, 24, 2))
        platforms.append(Platform(3600, 560, 300, 40, 0))
        enemies.append(Enemy(3150, 510, 80, hp(4)))
        enemies.append(FlyingEnemy(3400, 400, hp(3), float_height=350))
    ...
    # ── LEVEL 2: Plataformas — corrigido, sem espinhos duplos ─
        ...
        # Nova área expandida
        platforms.append(Platform(3500, 520, 200, 30, 2))
        platforms.append(Platform(3800, 450, 180, 24, 0))
        platforms.append(Platform(4100, 530, 300, 40, 1))
        enemies.append(ShootingEnemy(3550, 480, 80, hp(3)))
        enemies.append(Enemy(4150, 490, 100, hp(4)))

        goal = LevelGoal(4340, 444)
    ...
    # ── LEVEL 3: Puzzles — switches, portões, espinhos alternados ─
        ...
        # Nova área expandida
        platforms.append(Platform(3700, 540, 200, 30, 0))
        platforms.append(Platform(4000, 480, 180, 24, 2))
        platforms.append(Platform(4300, 560, 300, 40, 1))
        enemies.append(Enemy(3750, 500, 80, hp(4)))
        enemies.append(ShootingEnemy(4350, 520, 80, hp(4)))

        goal = LevelGoal(4650, 474)
    ...
    # ── LEVEL 4: A Ascensão — desafio de pulo vertical ───────
        ...
        # Nova área expandida
        platforms.append(Platform(4100, 500, 200, 30, 2))
        platforms.append(Platform(4400, 430, 180, 24, 0))
        platforms.append(Platform(4700, 520, 300, 40, 1))
        enemies.append(Enemy(4150, 460, 80, hp(5)))
        enemies.append(FlyingEnemy(4450, 350, hp(4), float_height=300))

        goal = LevelGoal(5080, 434)
    ...
    # ── LEVEL 5: O Boss Final — arena corrigida ───────────────
        ...
        # Wall at ARENA_X2 + 400 to allow player to reach the goal at 3880
