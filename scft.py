...
def build_level(level_id=1, difficulty=1.0):
    ...
    if level_id == 1:
        ...
        # Expanded content L1
        platforms.append(Platform(2700, 580, 400, 40, 1))
        platforms.append(Platform(3200, 520, 300, 40, 2))
        enemies.append(Enemy(2800, 540, 80, hp(4)))
        enemies.append(ShootingEnemy(2950, 540, 60, hp(3)))
        enemies.append(FlyingEnemy(3300, 400, hp(3), float_height=350))
        enemies.append(Enemy(3400, 480, 70, hp(4)))
        checkpoints.append(Checkpoint(3100, 536))
    ...
    # ── LEVEL 2: Plataformas — corrigido, sem espinhos duplos ─
        ...
        # Expanded content L2
        platforms.append(Platform(3000, 580, 300, 40, 0))
        m_plats.append(MovingPlatform(3400, 540, 150, 22, 250, 0))
        platforms.append(Platform(3750, 560, 400, 40, 2))
        enemies.append(Enemy(3050, 540, 80, hp(5)))
        enemies.append(ShootingEnemy(3150, 540, 70, hp(4)))
        enemies.append(FlyingEnemy(3500, 450, hp(3), float_height=380))
        enemies.append(Enemy(3800, 520, 90, hp(5)))
        enemies.append(ShootingEnemy(3950, 520, 80, hp(4)))
        checkpoints.append(Checkpoint(3700, 516))
    ...
    # ── LEVEL 3: Puzzles — switches, portões, espinhos alternados ─
        ...
        # Expanded content L3
        platforms.append(Platform(3100, 540, 300, 35, 1))
        platforms.append(Platform(3500, 540, 200, 30, 2, hazard=True, hazard_timed=True, hazard_offset=30))
        platforms.append(Platform(3800, 580, 500, 40, 0))
        enemies.append(Enemy(3150, 500, 70, hp(5)))
        enemies.append(ShootingEnemy(3250, 500, 70, hp(4)))
        enemies.append(FlyingEnemy(3600, 420, hp(3), float_height=350))
        enemies.append(Enemy(3850, 540, 90, hp(5)))
        enemies.append(ShootingEnemy(4000, 540, 80, hp(4)))
        enemies.append(Enemy(4200, 540, 80, hp(5)))
        checkpoints.append(Checkpoint(3750, 536))
    ...
    # ── LEVEL 4: A Ascensão — desafio de pulo vertical ───────
        ...
        # Expanded content L4
        platforms.append(Platform(3300, 480, 250, 30, 1))
        m_plats.append(MovingPlatform(3650, 440, 160, 22, 280, 0))
        platforms.append(Platform(4000, 500, 600, 40, 0))
        enemies.append(Enemy(3350, 440, 70, hp(6)))
        enemies.append(ShootingEnemy(3450, 440, 60, hp(5)))
        enemies.append(FlyingEnemy(3800, 350, hp(4), float_height=300))
        enemies.append(Enemy(4100, 460, 100, hp(6)))
        enemies.append(ShootingEnemy(4250, 460, 80, hp(5)))
        enemies.append(Enemy(4400, 460, 110, hp(6)))
        enemies.append(FlyingEnemy(4600, 380, hp(4), float_height=320))
        checkpoints.append(Checkpoint(3950, 456))

        goal = LevelGoal(5050, 464)

    # ── LEVEL 5: O Boss Final — arena corrigida ───────────────
    else:
        world_w, world_h = 4200, 900
        ...
        # Floor extends slightly beyond ARENA_X2 to accommodate the path to goal
        platforms.append(Platform(ARENA_X1, 560, ARENA_X2-ARENA_X1 + 400, 45, 0))

        # Paredes laterais da arena
        platforms.append(Platform(ARENA_X1-40, 0, 40, 900))   # parede esquerda arena
        # Boss Gate implemented as a barrier that opens on boss death
        bgate = Gate(ARENA_X2, 0, 40, 900)
        gates.append(bgate)
...
class Game:
    ...
    def _init_game(self):
        ...
        self.boss_gate_ref   = None
        if self.current_level == 5:
            # Em L5, o gate em ARENA_X2 é o boss gate
            for g in self.gates:
                if g.rect.x == 3600:
                    self.boss_gate_ref = g
    ...
    def update(self):
        ...
        # Boss gate logic
        if self.boss_gate_ref and self.hud.boss_ref:
            boss_dead = getattr(self.hud.boss_ref, "state", "") == getattr(Enemy, "DEAD", "dead")
            boss_timer = getattr(self.hud.boss_ref, "dead_timer", 0)
            if boss_dead and boss_timer >= 40:
                self.boss_gate_ref.update(True)
            else:
                self.boss_gate_ref.update(False)

        for g in self.gates:
            if g != self.boss_gate_ref:
                g.update(sw_active)
