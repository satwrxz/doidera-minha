...
def build_level(level_id=1, difficulty=1.0):
    ...
    if level_id == 1:
        ...
        platforms.append(Platform(3300, 520, 250, 30, 1))
        enemies.append(Enemy(3050, 510, 60, hp(4)))
        enemies.append(FlyingEnemy(3350, 450, hp(2), float_height=400))
    ...
    # ── LEVEL 2: Plataformas — corrigido, sem espinhos duplos ─
        ...
        platforms.append(Platform(3450, 580, 300, 40, 0))
        m_plats.append(MovingPlatform(3800, 540, 140, 22, 0, -150))
        platforms.append(Platform(4000, 580, 300, 40, 1))
        enemies.append(ShootingEnemy(3500, 540, 80, hp(3)))
    ...
    # ── LEVEL 3: Puzzles — switches, portões, espinhos alternados ─
        ...
        platforms.append(Platform(3650, 540, 250, 30, 1))
        platforms.append(Platform(4000, 580, 300, 40, 0))
        platforms.append(Platform(4350, 520, 200, 30, 2))
        enemies.append(ShootingEnemy(3700, 500, 70, hp(3)))
        enemies.append(FlyingEnemy(4100, 480, hp(3), float_height=420))
        enemies.append(Enemy(4400, 480, 60, hp(4)))
    ...
    # ── LEVEL 4: A Ascensão — desafio de pulo vertical ───────
        ...
        platforms.append(Platform(4000, 550, 200, 30, 2))
        platforms.append(Platform(4300, 500, 250, 30, 1))
        platforms.append(Platform(4650, 550, 300, 40, 0))
        enemies.append(ShootingEnemy(4050, 510, 80, hp(4)))
        enemies.append(FlyingEnemy(4350, 400, hp(3), float_height=350))
    ...
    # ── LEVEL 5: O Boss Final — arena corrigida ───────────────
        ...
        # O Gate agora age como a parede direita da arena, que abre após o boss
...
class Game:
    ...
    def update(self):
        ...
        # Lógica especial para Gate do Boss no Level 5
        if self.current_level == 5 and self.hud.boss_ref:
            boss = self.hud.boss_ref
            dead_state = getattr(Enemy, 'DEAD', 'dead')
            is_dead = getattr(boss, 'state', '') == dead_state
            dt = getattr(boss, 'dead_timer', 0)
            if is_dead and dt >= 40:
                for g in self.gates:
                    if g.rect.x >= 3600: # ARENA_X2
                        g.update(True)
    ...
    def _set_diff(self,val,label):
        self.difficulty=val; self._save_data(); self.hud.show_message(f"Dificuldade: {label}",70)
