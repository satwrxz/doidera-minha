...
def build_level(level_id=1, difficulty=1.0):
    ...
    if level_id == 1:
        ...
        # Expansão v2.0 (x > 2700)
        platforms.append(Platform(2960, 540, 300, 30, 2))
        platforms.append(Platform(3300, 580, 500, 40, 0))
        enemies.append(Enemy(3000, 500, 90, hp(4)))
        enemies.append(ShootingEnemy(3100, 500, 80, hp(3)))
        enemies.append(FlyingEnemy(3400, 450, hp(2), float_height=390))
    ...
    # ── LEVEL 2: Plataformas — corrigido, sem espinhos duplos ─
        ...
        # Expansão v2.0 (x > 3300)
        m_plats.append(MovingPlatform(3400, 540, 140, 22, 300, 0))
        platforms.append(Platform(3750, 580, 600, 40, 1))
        enemies.append(Enemy(3800, 540, 100, hp(4)))
        enemies.append(ShootingEnemy(3950, 540, 90, hp(3)))
        enemies.append(FlyingEnemy(4100, 460, hp(2), float_height=400))
    ...
    # ── LEVEL 3: Puzzles — switches, portões, espinhos alternados ─
        ...
        platforms.append(Platform(1230, 560, 350, 30, 2))
        # Dois espinhos com offsets bem separados (nunca ativos juntos)
        platforms.append(Platform(1600, 540, 150, 28, 1, hazard=True, hazard_timed=True, hazard_offset=0))
        platforms.append(Platform(1800, 540, 150, 28, 1, hazard=True, hazard_timed=True, hazard_offset=60))
        platforms.append(Platform(2000, 600, 300, 40, 0))

        enemies.append(ShootingEnemy(1300, 520, 80, hp(3)))
        enemies.append(Enemy(1400, 520, 60, hp(4)))
        enemies.append(ShootingEnemy(1650, 500, 60, hp(3)))
        enemies.append(FlyingEnemy(1800, 430, hp(2), float_height=380))
        enemies.append(Enemy(2050, 560, 80, hp(4)))

        orbs.append(HealthOrb(1900, 470))
        checkpoints.append(Checkpoint(1700, 516))

        # Zona 3: Moving platforms + combinação
        m_plats.append(MovingPlatform(2350, 540, 140, 22, 0, -160))
        platforms.append(Platform(2550, 580, 320, 40, 0))
        m_plats.append(MovingPlatform(2930, 560, 130, 22, 180, 0))
        platforms.append(Platform(3160, 560, 300, 30, 2))
        platforms.append(Platform(3520, 600, 540, 40, 0))

        # Expansão v2.0 (x > 4000)
        platforms.append(Platform(4100, 560, 600, 40, 1))
        enemies.append(Enemy(4200, 520, 100, hp(4)))
        enemies.append(ShootingEnemy(4350, 520, 80, hp(3)))
        enemies.append(FlyingEnemy(4500, 450, hp(2), float_height=390))

        enemies.append(Enemy(2400, 500, 80, hp(4)))
        enemies.append(ShootingEnemy(2580, 540, 80, hp(3)))
        enemies.append(FlyingEnemy(2800, 430, hp(3), float_height=370))
        enemies.append(Enemy(2980, 520, 70, hp(4)))
        enemies.append(Enemy(3200, 520, 80, hp(4)))
        enemies.append(ShootingEnemy(3300, 520, 70, hp(3)))
        enemies.append(Enemy(3560, 560, 120, hp(4)))
        enemies.append(FlyingEnemy(3700, 480, hp(2), float_height=400))
        enemies.append(Enemy(3800, 560, 100, hp(4)))

        orbs.append(HealthOrb(3200, 490))
        checkpoints.append(Checkpoint(3270, 536))
    ...
    # ── LEVEL 4: A Ascensão — desafio de pulo vertical ───────
        ...
        # Expansão v2.0 (x > 4000)
        platforms.append(Platform(4000, 550, 1000, 45, 0))
        enemies.append(Enemy(4100, 510, 120, hp(5)))
        enemies.append(ShootingEnemy(4250, 510, 100, hp(4)))
        enemies.append(Enemy(4400, 510, 120, hp(5)))
        enemies.append(FlyingEnemy(4600, 400, hp(3), float_height=340))
        enemies.append(ShootingEnemy(4750, 510, 100, hp(4)))
...
class Game:
    ...
    def update(self):
        ...
        if self.current_level == 5 and self.hud.boss_ref:
            ...
            if getattr(b, 'state', '') == dead_state and getattr(b, 'dead_timer', 0) > 40:
                sw_active = True
            ...
            for event in pygame.event.get():
                ...
                if event.type==pygame.KEYDOWN:
                    ...
                    if self.state==self.S_START:
                        ...
                        if event.key==pygame.K_k: self._reset_progress()
