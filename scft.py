...
def build_level(level_id=1, difficulty=1.0):
    ...
    if level_id == 1:
        ...
        # Conteúdo expandido L1
        platforms.append(Platform(2960, 520, 250, 30, 2))
        platforms.append(Platform(3250, 560, 300, 40, 1))
        enemies.append(Enemy(3000, 480, 80, hp(4)))
        enemies.append(FlyingEnemy(3300, 450, hp(2), float_height=380))
        enemies.append(Enemy(3400, 520, 100, hp(4)))
    ...
    # ── LEVEL 2: Plataformas — corrigido, sem espinhos duplos ─
        ...
        # Conteúdo expandido L2
        platforms.append(Platform(3360, 520, 240, 30, 1))
        platforms.append(Platform(3650, 560, 320, 40, 0))
        platforms.append(Platform(4000, 590, 400, 40, 2))
        enemies.append(ShootingEnemy(3400, 480, 80, hp(3)))
        enemies.append(Enemy(3700, 520, 100, hp(4)))
        enemies.append(FlyingEnemy(3800, 420, hp(3), float_height=350))
        enemies.append(Enemy(4100, 550, 120, hp(5)))
    ...
    # ── LEVEL 3: Puzzles — switches, portões, espinhos alternados ─
        ...
        # Conteúdo expandido L3
        platforms.append(Platform(3560, 540, 280, 30, 1))
        platforms.append(Platform(3880, 570, 340, 40, 0))
        platforms.append(Platform(4260, 540, 400, 40, 2))
        enemies.append(Enemy(3600, 500, 90, hp(5)))
        enemies.append(ShootingEnemy(3950, 530, 100, hp(4)))
        enemies.append(FlyingEnemy(4100, 420, hp(3), float_height=360))
        enemies.append(Enemy(4350, 500, 120, hp(5)))
    ...
    # ── LEVEL 4: A Ascensão — desafio de pulo vertical ───────
        ...
        # Conteúdo expandido L4
        platforms.append(Platform(3940, 500, 260, 30, 1))
        platforms.append(Platform(4250, 540, 320, 40, 0))
        platforms.append(Platform(4620, 530, 450, 40, 2))
        enemies.append(Enemy(4000, 460, 90, hp(5)))
        enemies.append(ShootingEnemy(4300, 500, 100, hp(5)))
        enemies.append(FlyingEnemy(4500, 400, hp(4), float_height=320))
        enemies.append(Enemy(4700, 490, 120, hp(6)))
        enemies.append(ShootingEnemy(4900, 490, 100, hp(5)))
...
class Game:
    ...
    def update(self):
        ...
        # Boss gate logic
        if self.hud.boss_ref:
            boss = self.hud.boss_ref
            boss_dead = getattr(boss, 'DEAD', 'dead')
            if getattr(boss, 'state', '') == boss_dead and getattr(boss, 'dead_timer', 0) >= 40:
                sw_active = True
