...
class Enemy:
    ...
    def update(self, platforms, player_rect):
        ...
        # Ledge detection and movement logic
        if self.on_ground and self.state in (self.PATROL, self.CHASE, self.ATTACK):
            ...
            if on_edge:
                if self.state == self.PATROL:
                    ...
                    # Chase/Attack: attempt to jump across gaps
...
def build_level(level_id=1, difficulty=1.0):
    ...
    if level_id == 1:
        ...
        platforms.append(Platform(3000, 550, 400, 30, 2))
        platforms.append(Platform(3500, 550, 450, 40, 1))
        enemies.append(Enemy(3100, 510, 100, hp(4)))
        enemies.append(FlyingEnemy(3300, 450, hp(2), float_height=380))
        enemies.append(Enemy(3600, 510, 120, hp(4)))
    ...
    # ── LEVEL 2: Plataformas — corrigido, sem espinhos duplos ─
        ...
        m_plats.append(MovingPlatform(3400, 540, 140, 22, 250, 0))
        platforms.append(Platform(3800, 590, 600, 40, 0))
        enemies.append(ShootingEnemy(3900, 550, 100, hp(3)))
        enemies.append(Enemy(4100, 550, 120, hp(4)))
        enemies.append(FlyingEnemy(4000, 450, hp(2), float_height=380))
    ...
    # ── LEVEL 3: Puzzles — switches, portões, espinhos alternados ─
        ...
        platforms.append(Platform(3600, 570, 400, 30, 2))
        platforms.append(Platform(4100, 570, 600, 40, 1))
        enemies.append(ShootingEnemy(3700, 530, 100, hp(3)))
        enemies.append(Enemy(4200, 530, 120, hp(4)))
        enemies.append(FlyingEnemy(4300, 450, hp(2), float_height=380))
    ...
    # ── LEVEL 4: A Ascensão — desafio de pulo vertical ───────
        ...
        platforms.append(Platform(4000, 540, 1100, 40, 0))
        enemies.append(Enemy(4100, 500, 120, hp(5)))
        enemies.append(ShootingEnemy(4300, 500, 100, hp(4)))
        enemies.append(Enemy(4500, 500, 120, hp(5)))
        enemies.append(FlyingEnemy(4700, 400, hp(3), float_height=340))
        enemies.append(ShootingEnemy(4900, 500, 80, hp(4)))
    ...
    # ── LEVEL 5: O Boss Final — arena corrigida ───────────────
        ...
        # Extend floor beyond ARENA_X2 to allow path to goal
...
class Game:
    ...
    def update(self):
        ...
        # Switches / Gates
        if not self.boss_defeated_gate:
            for e in self.enemies:
                if isinstance(e, Boss) and getattr(e, 'state', '') == getattr(Enemy, 'DEAD', 'dead'):
                    self.boss_defeated_gate = True
                    break
        sw_active=any(s.active for s in self.switches) or self.boss_defeated_gate
