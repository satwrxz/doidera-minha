...
class Enemy:
    ...
    def update(self, platforms, player_rect):
        ...
        # Borda de plataforma: NUNCA pula em PATROL, tenta pular em CHASE/ATTACK
        if self.on_ground and self.state in (self.PATROL, self.CHASE, self.ATTACK):
            ...
            if on_edge:
                if self.state == self.PATROL:
                    self.facing *= -1
                    self.vx = self.patrol_spd * self.facing
                else: # CHASE ou ATTACK
                    self.vy = 0.75 * JUMP_FORCE # Tenta pular o gap
...
def build_level(level_id=1, difficulty=1.0):
    ...
    if level_id == 1:
        ...
        # Expansão x > 2700
        platforms.append(Platform(3000, 540, 200, 24, 2))
        platforms.append(Platform(3300, 600, 600, 40, 0))
        enemies.append(Enemy(3100, 500, 60, hp(4)))
        enemies.append(Enemy(3400, 560, 100, hp(4)))
        enemies.append(FlyingEnemy(3600, 520, hp(2), float_height=450))

        goal = LevelGoal(3800, 514)
    ...
    # ── LEVEL 2: Plataformas — corrigido, sem espinhos duplos ─
        ...
        # Expansão x > 2700
        m_plats.append(MovingPlatform(3000, 450, 130, 22, 0, 150))
        platforms.append(Platform(3200, 580, 400, 40, 1))
        platforms.append(Platform(3700, 540, 250, 30, 2, hazard=True, hazard_timed=True, hazard_offset=30))
        platforms.append(Platform(4050, 600, 400, 40, 0))

        enemies.append(Enemy(3300, 540, 80, hp(4)))
        enemies.append(ShootingEnemy(3400, 540, 70, hp(3)))
        enemies.append(FlyingEnemy(3800, 450, hp(3), float_height=380))
        enemies.append(Enemy(4150, 560, 100, hp(4)))

        goal = LevelGoal(4300, 514)
    ...
    # ── LEVEL 3: Puzzles — switches, portões, espinhos alternados ─
        ...
        platforms.append(Platform(600, 560, 450, 30, 1))
        ...
        # Expansão x > 2700
        platforms.append(Platform(2700, 560, 400, 30, 1))
        switches.append(Switch(2900, 524))
        gates.append(Gate(3100, 360, 40, 240))
        platforms.append(Platform(3200, 600, 500, 40, 0))
        m_plats.append(MovingPlatform(3800, 540, 140, 22, 200, 0))
        platforms.append(Platform(4100, 580, 600, 40, 0))

        enemies.append(Enemy(2800, 520, 80, hp(4)))
        enemies.append(ShootingEnemy(3300, 560, 100, hp(4)))
        enemies.append(FlyingEnemy(3500, 450, hp(3), float_height=380))
        enemies.append(Enemy(4200, 540, 120, hp(5)))
        enemies.append(ShootingEnemy(4400, 540, 80, hp(4)))

        goal = LevelGoal(4600, 494)
    ...
    # ── LEVEL 4: A Ascensão — desafio de pulo vertical ───────
        ...
        # Expansão x > 2700
        platforms.append(Platform(4000, 500, 200, 24, 2))
        platforms.append(Platform(4300, 440, 200, 24, 1))
        platforms.append(Platform(4600, 600, 500, 40, 0))

        enemies.append(Enemy(4100, 460, 60, hp(5)))
        enemies.append(FlyingEnemy(4400, 350, hp(4), float_height=300))
        enemies.append(ShootingEnemy(4700, 560, 120, hp(5)))
        enemies.append(Enemy(4850, 560, 100, hp(5)))

        goal = LevelGoal(5000, 514)
    ...
    # ── LEVEL 5: O Boss Final — arena corrigida ───────────────
        ...
        # Extende o chão 400px além de ARENA_X2
        platforms.append(Platform(ARENA_X1, 560, (ARENA_X2 + 400) - ARENA_X1, 45, 0))
...
class Game:
    ...
    def draw(self):
        ...
        if self.entry_fade > 0:
            self.entry_fade = max(0, self.entry_fade - 3)
